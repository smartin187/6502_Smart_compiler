# To Do

The futures feature of Smart, or fix bugs.

If the task is a fix, the text is _italic_.

If the task is a important feature, the text is **bold**.

The task with `?` is task that is not sure if it will be implemented.

If the task is a bug you can also add a issue on GitHub.

## Tasks

- _Fix: bug with return function if the function is call in a mathematical operation (`.a = 1 + f:;`)_
- _Fix: small bug on windows: the path of library are not join correctly: ...lib\global_lib\screen_tool/screen_tool.sma ; moreover a critical message is show if a library is not find in a path, but the library is find in another path._
- **Add function with arguments for Smart**
- Add constants (know at compile time), or add define.
- Make the libraries for Smart: `math` ?, `string`, library for write in RAM/ROM ?
- Add on test.py test for all library
- add a function for restart programme (JMP #0400)
- **Add string and list with size variable ?**
- Add class object for Smart ?
- Multi-threading for Smart ?
